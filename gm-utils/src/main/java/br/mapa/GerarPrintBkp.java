...
public class GerarPrintBkp {
	
	private static boolean isPrimitiva(String s) {
		
		if ("Date".contentEquals(s)) {
			return true;
		}

		if ("BigDecimal".contentEquals(s)) {
			return true;
		}
	...
	private static GFile mainPath = GFile.get("C:\\desenvolvimento\\projetos\\funcafe-src\\funcafe-ejb\\src\\main\\java\\br\\gov\\mapa\\funcafe");
...
//		imports.addIfNotContains("import java.util.List;");
	...
	private static String getImp(GFile file, ListString importsOriginais, String tipo, String classeFull) {
		...
		if (x != null) {
			...
			return x;
		} else if (file.getPath().join(tipo + ".java").exists()) {
