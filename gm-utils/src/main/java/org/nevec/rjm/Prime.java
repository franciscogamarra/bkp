...
public class Prime {
	...
	public int millerRabin(BigInteger n) {
		 ...
		 */
		String[] mr = { "2047", "1373653", "25326001", "3215031751", "2152302898747", "3474749660383",
				"341550071728321" };
		int mrLim = 0;
	...
	protected void growto(BigInteger n) {
		while (nMax.compareTo(n) < 0) {
			...
			boolean isp = true;
			for (BigInteger element : a) {
				/*
				 * Test the list of known primes only up to sqrt(n)
				 */
				if (element.multiply(element).compareTo(nMax) > 0) {
					break;
				}
				 ...
				 */
				if (nMax.remainder(element).compareTo(BigInteger.ZERO) == 0) {
					isp = false;
					break;
				}
			}
	...
	public static void main(String[] args) {
		Prime a = new Prime();
		int n = (IntegerParse.toInt(args[0]));
		if (n >= 1) {
			if (n >= 2) {
				console.log("prime(" + (n - 1) + ") = " + a.at(n - 1));
			}
			console.log("prime(" + n + ") = " + a.at(n));
			console.log("prime(" + (n + 1) + ") = " + a.at(n + 1));
			console.log("pi(" + n + ") = " + a.pi(new BigInteger("" + n)));
		}
	}
} /* Prime */
