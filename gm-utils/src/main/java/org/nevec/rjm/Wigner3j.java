...
public class Wigner3j {
	...
	public static void main(String args[]) {
		if (args[0].compareTo("6j") == 0) {
			...
			int j1 = IntegerParse.toInt(args[1]);
			int j2 = IntegerParse.toInt(args[2]);
			int j3 = IntegerParse.toInt(args[3]);
			int m1 = IntegerParse.toInt(args[4]);
			int m2 = IntegerParse.toInt(args[5]);
			int m3 = IntegerParse.toInt(args[6]);
			try {
				BigSurd w = wigner3jm(j1, j2, j3, m1, m2, m3);
				console.log(w.toString());
				w = w.multiply(new BigSurd(j3 + 1, 1));
				console.log("CG factor sqrt" + (j3 + 1) + "sign " + ((j2 - j2 - m3) / 2) + " " + w.toString());
			} catch (Exception e) {
				console.log(e.getMessage());
			}
		} else {
			console.log("usage:");
			console.log(args[0] + " 6j 2j1+1 2j2+1 2j3+1 2j4+1 2j5+1 2j6+1");
			console.log(args[0] + " 9j 2j1+1 2j2+1 2j3+1 2j4+1 2j5+1 2j6+1.. 2j9+1 ");
			console.log(args[0] + " 3jm 2j1+1 2j2+1 2j3+1 2m1+1 2m2+1 2m3+1 ");
		}
	} /* Wigner3j.main */
	 ...
	 * @author Richard J. Mathar
	 */
	static private BigSurdVec wigner3j(int[] tvec, Rational[] J, Rational[] M, int[] triadidx) {
		/*
		...
		for (int t = 0; t < triadidx.length; t += 3) {
			...
			if ((J[triadidx[t]].subtract(J[triadidx[t + 1]]).abs().compareTo(J[triadidx[t + 2]]) > 0) || (J[triadidx[t]].add(J[triadidx[t + 1]]).compareTo(J[triadidx[t + 2]]) < 0)) {
				return res;
			}
		}
	...
	static protected BigSurd wigner3jm(Rational j1, Rational j2, Rational j3, Rational m1, Rational m2, Rational m3) {
		 ...
		 */
		if ((m1.add(m2).add(m3).signum() != 0) || !j1.add(j2).add(j3).isBigInteger()) {
			return BigSurd.ZERO;
		}
