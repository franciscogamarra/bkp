...
public class Ifactor implements Cloneable, Comparable<Ifactor> {
	...
	public static Ifactor ONE = new Ifactor(1);

	public static Ifactor ZERO = new Ifactor(0);
...
//		Vector<Integer> p = (Vector<Integer>) primeexp.clone();
	...
	protected Ifactor multGcdLcm(Ifactor oth, int type) {
		...
		if (primeexp.size() != 0 && oth.primeexp.size() != 0) {
			...
			if (primeexp.firstElement().intValue() == 1 && type == 2) {
				return oth;
			} else if ((primeexp.firstElement().intValue() == 1 && type == 1) || (oth.primeexp.firstElement().intValue() == 1 && type == 0)) {
				return this;
	...
	public Vector<BigInteger> divisors() {
		...
		if (n.compareTo(BigInteger.ONE) > 0) {
			...
			/* get ez */
			int ez = primeexp.lastElement();

			Vector<BigInteger> partd = dp.divisors();

			/* obtain pz by lookup in the prime list */
			BigInteger pz = new BigInteger(primeexp.elementAt(primeexp.size() - 2).toString());
