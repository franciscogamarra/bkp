...
public class BigDecimalMath {
	...
	public static BigDecimal sqrt(BigDecimal x, MathContext mc) {
		...
		for (;;) {
			...
			/*
			 * debugging console.log("itr "+x.round(locmc).toString() + " " +
	...
	public static BigDecimal root(int n, BigDecimal x) {
		 ...
		 */
		double eps = x.ulp().doubleValue() / (2 * n * x.doubleValue());
		for (;;) {
			/*
	...
	public static BigDecimal log(int n, MathContext mc) {
		...
		if (n == 1) {
			 ...
			 */
			MathContext mcloc = new MathContext(1 + err2prec(n, eps));
			/*
	...
	public static BigDecimal log(Rational r, MathContext mc) {
		...
		if (r.compareTo(Rational.ONE) == 0) {
			...
			BigDecimal resul = log(r.BigDecimalValue(mcloc));
	...
	public static BigDecimal tan(BigDecimal x) {
		...
		if (x.compareTo(BigDecimal.ZERO) < 0) {
			...
			double xUlpDbl = x.ulp().doubleValue() / 2.;
			double eps = xUlpDbl / 2. / Math.pow(Math.cos(xDbl), 2.);
			...
			if (xDbl > 0.8) {
				...
				BigDecimal xhighpr = scalePrec(res, 2);
	...
	public static BigDecimal cot(BigDecimal x) {
		...
		if (x.compareTo(BigDecimal.ZERO) < 0) {
			...
			double xDbl = res.doubleValue();
			double xUlpDbl = x.ulp().doubleValue() / 2.;
			double eps = xUlpDbl / 2. / Math.pow(Math.sin(xDbl), 2.);
	...
	public static BigDecimal asin(BigDecimal x) {
		...
		if (x.compareTo(BigDecimal.ZERO) == 0) {
			...
			double xDbl = x.doubleValue();
			double xUlpDbl = x.ulp().doubleValue() / 2.;
			double eps = xUlpDbl / 2. / Math.sqrt(1. - Math.pow(xDbl, 2.));

			BigDecimal xhighpr = scalePrec(xCompl, 3);
	...
	public static BigDecimal atan(BigDecimal x) {
		...
		if (x.compareTo(BigDecimal.ZERO) == 0) {
			...
			BigDecimal xhighpr = scalePrec(x, 2);
			BigDecimal xhighprSq = multiplyRound(xhighpr, xhighpr).negate();
			...
			BigDecimal xhighpr = divideRound(-1, scalePrec(x, 2));
			BigDecimal xhighprSq = multiplyRound(xhighpr, xhighpr).negate();
	 ...
	 * @author Richard J. Mathar
	 */
	public static BigDecimal Gamma(Rational q, MathContext mc) {
		if (q.isBigInteger()) {
	...
	public static BigDecimal pochhammer(BigDecimal x, int n) {
		...
		if (n == 0) {
			...
			for (int i = 1; i < n; i++) {
				...
				MathContext mcloc = new MathContext(4 + err2prec(eps));
	...
	public static double zeta1(int n) {
		 ...
		 */
		double[] zmin1 = { 0., 0., 6.449340668482264364724151666e-01, 2.020569031595942853997381615e-01,
	...
	public static double psi(double x) {
		...
		if (Math.abs(x - psi0) < 0.55) {
			/*
			 * Taylor approximation around the local zero
			 */
			double[] psiT0 = { 9.67672245447621170427e-01, -4.42763168983592106093e-01,
	 ...
	 * @author Richard J. Mathar
	 */
	static protected BigDecimal broadhurstBBP(int n, int p, int a[], MathContext mc) {
		/*
	 ...
	 * @author Richard J. Mathar
	 */
	public static Vector<BigInteger> cfrac(BigDecimal x) {
		/*
	...
	public static BigComplex addRound(BigComplex x, BigComplex y) {
		BigDecimal R = addRound(x.re, y.re);
		BigDecimal I = addRound(x.im, y.im);
	...
	public static BigComplex subtractRound(BigComplex x, BigComplex y) {
		BigDecimal R = subtractRound(x.re, y.re);
		BigDecimal I = subtractRound(x.im, y.im);
		return new BigComplex(R, I);
	} /* subtractRound */
	...
	public static BigComplex invertRound(BigComplex z) {
		if (z.im.compareTo(BigDecimal.ZERO) == 0) {
			 ...
			 */
			MathContext mc = new MathContext(z.re.precision());
			return new BigComplex(BigDecimal.ONE.divide(z.re, mc));
		}
		if (z.re.compareTo(BigDecimal.ZERO) == 0) {
			/*
			 * In this case with vanishing Re(z), the result is simply -i/Im z
			 */
			MathContext mc = new MathContext(z.im.precision());
	...
	public static BigComplex divideRound(BigInteger n, BigComplex x) {
		...
		if (x.re.compareTo(BigDecimal.ZERO) == 0) {
