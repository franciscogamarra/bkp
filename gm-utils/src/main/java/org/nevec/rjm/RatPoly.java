...
class RatPoly {
	...
	public RatPoly add(RatPoly val) {
		 ...
		 * at least).
		 */
		int nmax = (degree() > val.degree()) ? degree() : val.degree();
		for (int n = 0; n <= nmax; n++) {
	...
	public String toPString() {
		String str = new String();
		for (int n = 0; n < a.size(); n++) {
			BigInteger num = a.elementAt(n).a;
	...
	public RatPoly derive() {
		...
		for (int i = 1; i <= degree(); i++) {
			Rational c = a.elementAt(i).multiply(i);
			d.set(i - 1, c);
		}
		return d;
	} /* derive */
	...
	public RatPoly monic() {
		...
		int d = degree();
		for (int i = 0; i <= d; i++) {
			Rational c = a.elementAt(i).divide(a.elementAt(d));
			m.set(i, c);
		}
		return m;
	} /* monic */
