...
public class BigIntegerMath {
	...
	public static BigInteger binomial(int n, int k) {
		...
		BigInteger n2 = bin;
		for (BigInteger i = BigInteger.valueOf(k - 1); i.compareTo(BigInteger.ONE) >= 0; i = i
				.subtract(BigInteger.ONE)) {
			bin = bin.multiply(n2.subtract(i));
		}
		for (BigInteger i = BigInteger.valueOf(k); i.compareTo(BigInteger.ONE) > 0; i = i
				.subtract(BigInteger.ONE)) {
			bin = bin.divide(i);
		}
		return (bin);
	...
	public static BigInteger iroot(BigInteger x, int n) {
		...
		if (bl > 120) {
			...
			double resul = Math.pow(x.doubleValue(), 1.0 / n);
		...
		if (rL == 2) {
			...
			for (int r = 0; r < rL; r++) {
				...
				if (A[r][0].compareTo(BigInteger.ZERO) != 0) {
					BigInteger M[][] = minor(A, r, 0);
					BigInteger m = A[r][0].multiply(det(M));
					/* recursive call */
					if (r % 2 == 0) {
						d = d.add(m);
					} else {
