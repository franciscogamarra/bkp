...
public class BigComplex {
	 ...
	 */
	static BigComplex ZERO = new BigComplex(BigDecimal.ZERO, BigDecimal.ZERO);
	...
	BigComplex multiply(BigComplex oth, MathContext mc) {
		BigDecimal a = re.add(im).multiply(oth.re);
		BigDecimal b = oth.re.add(oth.im).multiply(im);
		BigDecimal c = oth.im.subtract(oth.re).multiply(re);
	...
	BigComplex subtract(BigComplex oth) {
		BigDecimal x = re.subtract(oth.re);
		BigDecimal y = im.subtract(oth.im);
		return new BigComplex(x, y);
	}
	...
	BigComplex sqrt(MathContext mc) {
		...
		BigDecimal u = BigDecimalMath.sqrt(l.add(re).divide(half, mc), mc);
		BigDecimal v = BigDecimalMath.sqrt(l.subtract(re).divide(half, mc), mc);
