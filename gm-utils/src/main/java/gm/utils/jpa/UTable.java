...
import gm.utils.jpa.constructor.CreateTable;
...
public class UTable {
	...
	UTable(ConexaoJdbc conexao, Class<?> classe){
		con = conexao;
		this.classe = classe;
		ts = TableSchema.get(con.getDefaultSchema(), classe);
		schema = StringBeforeFirst.get(ts, ".");
		name = StringAfterFirst.get(ts, ".");
	}
	...
	public int maxId() {
		...
		return IntegerParse.toIntDef(con.selectInt(s),0);
	}
	public boolean exists(int id) {
		if (classe == null) {
			throw new RuntimeException("classe == null");
		}
		String columnName = AtributosBuild.get(classe).getId().getColumnName();
	...
	private Integer getId(Object o) {
		return AtributosBuild.get(o).getId().get(o);
	}
	private void setId(Object o, Integer id) {
		AtributosBuild.get(o).getId().set(o, id);
	}
	...
	private String nativeValue(Atributo a, Object o) {
		...
		if (Null.is(x)) {
			...
			if (valorSeNulo != null) {
				x = valorSeNulo.value();
			} else if ("excluido".equals(a.nome()) || "somenteLeitura".equals(a.nome())) {
		...
		if ( (con.getDriver() == DriverJDBC.MSSQLServer) && a.isBoolean() ) {
		...
		return AtributosBuild.getPersists(x.getClass(), false).getId().get(x).toString();

	}

	private <T> T insert(T o, Integer id) {

		boolean semSequence = !AtributosBuild.get(o.getClass()).getId().hasAnnotation(GeneratedValue.class);

		if (semSequence) {
			if (id == null) {
				id = getId(o);
				if (id == null) {
					id = IntegerParse.toIntDef(con.selectInt("select max(id) from " + ts),0) + 1;
					setId(o, id);
				}
			}
		} else if (id != null) {
		...
		for (Atributo a : as) {
			...
			if (StringEmpty.is(nativeValue) || "null".equals(nativeValue)) {
				if (!"excluido".equals(a.nome()) && !"somenteLeitura".equals(a.nome())) {
					continue;
				}
				if (con.getDriver() == DriverJDBC.MSSQLServer) {
					nativeValue = "0";
		...
		if (id == null) {
			...
			con.exec(s);
			id = IntegerParse.toIntDef(con.selectInt("select max(id) from " + ts),0);
			setId(o, id);
		} else {
	...
	public void drop(){
		if (exists()) {
			ListString colunas = con.selectStrings("select coluna from jdbc.colunas where lower(esquema) = '"+getSchema().toLowerCase()+"' and lower(tabela) = '"+getName().toLowerCase()+"'");
			
			if (colunas.size() > 1) {
				for(String coluna: colunas) {
					con.tryExec("alter table " + ts + " drop column " + coluna);
				}
			}
...
//		con.exec(sql);
//		exists = true;
//		Atributos as = Atributos.persist(classe);
//		for (Atributo a : as) {
...
//	}
	...
	public void ajustSequence() {
		...
		x++;
		String s = StringAfterFirst.get(ts, ".");
		s += "_id_seq";
	...
	public Integer selectInt(String where) {
		return selectUniqueAs(where);
	}
	...
	public void create() {
		new CreateTable(con).create(classe);
	}
}
