package gm.utils.jpa.constructor;
import java.util.Calendar;
import java.util.List;

import gm.utils.classes.UClass;
import gm.utils.classes.interfaces.ClasseReal;
import gm.utils.classes.interfaces.IClass;
import gm.utils.classes.interfaces.IField;
import gm.utils.comum.Lst;
import gm.utils.date.Data;
import gm.utils.javaCreate.JcClasse;
import gm.utils.javaCreate.JcMetodo;
import gm.utils.javaCreate.JcParametro;
import gm.utils.javaCreate.JcTipo;
import gm.utils.jpa.criterions.Criterio;
import gm.utils.jpa.select.SelectBase;
import gm.utils.jpa.select.SelectBoolean;
import gm.utils.jpa.select.SelectDataComHora;
import gm.utils.jpa.select.SelectDate;
import gm.utils.jpa.select.SelectEnum;
import gm.utils.jpa.select.SelectInteger;
import gm.utils.jpa.select.SelectLong;
import gm.utils.jpa.select.SelectNumeric1;
import gm.utils.jpa.select.SelectNumeric15;
import gm.utils.jpa.select.SelectNumeric18;
import gm.utils.jpa.select.SelectNumeric2;
import gm.utils.jpa.select.SelectNumeric3;
import gm.utils.jpa.select.SelectNumeric4;
import gm.utils.jpa.select.SelectNumeric5;
import gm.utils.jpa.select.SelectNumeric8;
import gm.utils.jpa.select.SelectShort;
import gm.utils.jpa.select.SelectString;
import gm.utils.lambda.F1;
import lombok.Setter;
import src.commom.utils.string.StringEmpty;

@Setter
public class CriarSelect {

	private IClass classe;
	private boolean pojo;
	private boolean update;
	private IClass classeId = ClasseReal.get(Long.class);
	private boolean salvar = true;
	private String pathJavaMain = System.getProperty("user.dir") + "/src/main/java/";
	private String pacoteOndeAsClassesSeraoCriadas;
	private String sn;
	private JcClasse jc;
	private Class<?> classeDia;

	public JcClasse exec() {

		if (StringEmpty.is(pacoteOndeAsClassesSeraoCriadas)) {
			throw new NullPointerException("pacoteOndeAsClassesSeraoCriadas is null");
		}
		
		JcTipo ORIGEM = JcTipo.GENERICS("ORIGEM");

		sn = classe.getSimpleName();
		jc = JcClasse.build(pathJavaMain, pacoteOndeAsClassesSeraoCriadas, sn + "Select");
		jc.addGenerics(ORIGEM);

		{
			JcTipo o = new JcTipo(SelectBase.class);
			o.addGenerics(ORIGEM);
			o.addGenerics(classe);
			o.addGenerics(new JcTipo(jc, ORIGEM));
			jc.extends_(o);
		}

		jc.addImport(Criterio.class);
		jc.addImport(SelectBase.class);
		jc.addImport(classe);

		String n = sn;
		
		/* deixar ordenado pelo nome para que mudancas na posicao do campo nao causem mudandas no codigo */
		Lst<IField> as = classe.getFields().filter(i -> i.isPersistent() && !i.isId()).sortString(i -> i.getName());
		
		F1<String, Boolean> anyField = s -> as.anyMatch(i -> i.eq(s));

		jc.construtor()
		.public_()
		.param("origem", ORIGEM)
		.param(new JcParametro("criterio", Criterio.class).generics("?"))
		.param("prefixo", String.class)
		.add("super(origem, criterio, prefixo, " + n + ".class);");

		if (anyField.call("ordem") || anyField.call("nome")) {
			JcMetodo m = jc.metodo("beforeSelect").override();
			if (anyField.call("ordemHierarquia")) {
				m.add("ordemHierarquia().asc();");
			}
			if (anyField.call("ordem")) {
				m.add("ordem().asc();");
			}
			if (anyField.call("nome")) {
				m.add("nome().asc();");
			}
		}

		if (pojo) {
			jc.metodo("pojo").public_().type(pacoteOndeAsClassesSeraoCriadas + "."+n+"Pojo").add("return new " + n + "Pojo(list());");
		}

		if (update) {
			jc.metodo("update").public_().type(pacoteOndeAsClassesSeraoCriadas + "."+n+"Update").add("return new " + n + "Update(this);");
		}

		if (classeDia != null) {
			Class<?> fwTabela = UClass.getClass("gm.fw.model.Tabela");
			if (classe.eq(fwTabela)) {
				Class<?> fwEntidade = UClass.getClass("gm.fw.support.Entidade");
				jc.addImport(fwEntidade);
				Class<?> fwTabelaBoo = UClass.getClass("gm.fw.boos.TabelaBoo");
				jc.addImport(fwTabelaBoo);

				jc.metodo("eq").public_().type(ORIGEM)
				.param(new JcParametro("classe", Class.class).generics("? extends Entidade<?>"))
				.add("return eq(TabelaBoo.get(classe));");
			}
		}
		
		as.add(0, classe.getId());
		for (IField a : as) {
			
			String s = a.getName();

			String nomeValue = a.getNomeValue();

			JcMetodo m = jc.metodo(s).public_();
			
			if (a.isId()) {
				m.override();
			}
			
			if (a.isInteger()) {
				m.type(SelectInteger.class).add("return new SelectInteger<>(this, \"" + nomeValue + "\");");
			} else if (a.isLong() || a.is(Long.class)) {
				m.type(SelectLong.class).add("return new SelectLong<>(this, \"" + nomeValue + "\");");
			} else if (a.isShort()) {
				m.type(SelectShort.class).add("return new SelectShort<>(this, \"" + nomeValue + "\");");
			} else if (a.isBoolean()) {
				m.type(SelectBoolean.class).add("return new SelectBoolean<>(this, \"" + nomeValue + "\");");
			} else if (a.isString() ) {
				m.type(SelectString.class).add("return new SelectString<>(this, \"" + nomeValue + "\");");
			} else if (a.isEnum()) {
				m.type(SelectEnum.class);
				m.add("return new SelectEnum<>(this, \"" + nomeValue + "\");");
			} else if (a.isFk()) {
				String tp = a.getType().getSimpleName();
				m.type(pacoteOndeAsClassesSeraoCriadas + "."+tp+"Select");
				m.add("return new "+tp+"Select<>(this, getC(), getPrefixo() + \"." + s + "\" );");
			} else if (a.isDate()) {
				m.type(SelectDate.class);
				m.add("return new SelectDate<>(this, \"" + nomeValue + "\");");
			} else if (a.isDataComHora()) {
				m.type(SelectDataComHora.class);
				m.add("return new SelectDataComHora<>(this, \"" + nomeValue + "\");");
			} else if (a.isNumeric2()) {
				m.type(SelectNumeric2.class);
				m.add("return new SelectNumeric2<>(this, \""+nomeValue+"\");");
			} else if (a.isNumeric1()) {
				m.type(SelectNumeric1.class);
				m.add("\t\treturn new SelectNumeric1<>(this, \""+nomeValue+"\");");
			} else if (a.isNumeric3()) {
				m.type(SelectNumeric3.class);
				m.add("return new SelectNumeric3<>(this, \""+nomeValue+"\");");
			} else if (a.isNumeric4()) {
				m.type(SelectNumeric4.class);
				m.add("return new SelectNumeric4<>(this, \""+nomeValue+"\");");
			} else if (a.isNumeric5()) {
				m.type(SelectNumeric5.class);
				m.add("return new SelectNumeric5<>(this, \""+nomeValue+"\");");
			} else if (a.isNumeric8()) {
				m.type(SelectNumeric8.class);
				m.add("return new SelectNumeric8<>(this, \""+nomeValue+"\");");
			} else if (a.isNumeric15()) {
				m.type(SelectNumeric15.class);
				m.add("return new SelectNumeric15<>(this, \""+nomeValue+"\");");
			} else if (a.isNumeric18()) {
				m.type(SelectNumeric18.class);
				m.add("return new SelectNumeric18<>(this, \""+nomeValue+"\");");
			} else {
				throw new RuntimeException("??? " + a);
			}

			JcTipo genericsRetorno = new JcTipo(pacoteOndeAsClassesSeraoCriadas + "." + n + "Select");
			genericsRetorno.addGenerics("?");
			m.addGenericsRetorno(genericsRetorno);
			
			if (a.isEnum()) {
				m.addGenericsRetorno(a.getType());
			}

		}
		
		Lst<IField> datas = as.filter(i -> i.is(Calendar.class));
		
		if (datas.anyMatch(i -> i.eq("inicio")) && datas.anyMatch(i -> i.eq("fim"))) {
			JcMetodo m = jc.metodo("entreInicioEFim").public_();
			m.type(pacoteOndeAsClassesSeraoCriadas + "."+n+"Select");
			m.addGenericsRetorno(ORIGEM);
			m.param("data", Calendar.class);
			m.add("inicio().menorOuIgual(data);");
			m.add("fim().maiorOuIgual(data);");
			m.add("return this;");
		}
		if (classeDia != null) {
			Lst<IField> dias = as.filter(i -> i.getType().eq(classeDia));
			if (dias.anyMatch(i -> i.eq("inicio")) && dias.anyMatch(i -> i.eq("fim"))) {
				JcMetodo m = jc.metodo("entreInicioEFim").public_();
				m.addGenericsRetorno("ORIGEM");
				m.type(n+"Select");
				m.param("data", Data.class);
				m.add("inicio().menorOuIgual(data);");
				m.add("fim().maiorOuIgual(data);");
				m.add("return this;");
			}
		}
		
		IField pai = as.unique(i -> i.eq("pai"));
		if (pai != null && pai.is(classe)) {
			
			IField hierarquia = as.unique(i -> i.eq("hierarquia"));
			
			if (hierarquia != null && hierarquia.is(String.class)) {
				jc.addImport(List.class);
				JcMetodo m = jc.metodo("hierarquia_of").public_();
				m.type("gm.auto."+n+"Select<?>");
				m.param("list", List.class, classe);
				m.add("while (!list.isEmpty()) {");
				m.add(classe.getSimpleName()+" o = list.remove(0);");
				m.add("hierarquia().like(o.getHierarquia()+\";\");");
				m.add("if (!list.isEmpty()) or();");
				m.add("}");
				m.add("return this;");
			}
		}

		JcTipo tipo = new JcTipo(pacoteOndeAsClassesSeraoCriadas + "."+n+"Select");
		tipo.addGenerics("?");

		jc.metodo("asc").public_().type(tipo).add("return id().asc();");
		jc.metodo("desc").public_().type(tipo).add("return id().desc();");

		if (classeDia != null && classe.eq(classeDia)) {

			jc.addImport(pacoteOndeAsClassesSeraoCriadas + ".FwSelect");
			jc.addImport(Calendar.class);
			jc.addImport(Data.class);

			jc.metodo("min").public_().type(classeDia)
			.add(classeId.getSimpleName() + " id = id().min();")
			.add("if (id == null) return null;")
			.add("return byId(id);");

			jc.metodo("max").public_().type(classeDia)
			.add(classeId.getSimpleName() + " id = id().max();")
			.add("if (id == null) return null;")
			.add("return byId(id);");

			jc.metodo("maior").public_().type(ORIGEM).param("dia", classeDia)
			.add("id().maior(dia.getId());")
			.add("return getOrigem();");

			jc.metodo("maiorOuIgual").public_().type(ORIGEM).param("dia", classeDia)
			.add("id().maiorOuIgual(dia.getId());")
			.add("return getOrigem();");

			jc.metodo("menorOuIgual").public_().type(ORIGEM).param("dia", classeDia)
			.add("id().menorOuIgual(dia.getId());")
			.add("return getOrigem();");

			jc.metodo("maiorOuIgual").public_().type(ORIGEM).param("dia", Calendar.class)
			.add("data().maiorOuIgual(dia);")
			.add("return getOrigem();");

			jc.metodo("menorOuIgual").public_().type(ORIGEM).param("dia", Calendar.class)
			.add("data().menorOuIgual(dia);")
			.add("return getOrigem();");

			jc.metodo("maiorOuIgual").public_().type(ORIGEM).param("dia", Data.class)
			.add("data().maiorOuIgual(dia);")
			.add("return getOrigem();");

			jc.metodo("menorOuIgual").public_().type(ORIGEM).param("dia", Data.class)
			.add("data().menorOuIgual(dia);")
			.add("return getOrigem();");

			jc.metodo("menor").public_().type(ORIGEM).param("dia", classeDia)
			.add("id().menor(dia.getId());")
			.add("return getOrigem();");

		}

		jc.metodo("lst").public_().type(Lst.class, classe).return_("list()");

		if (salvar) {
			jc.save();
		}

		return jc;

	}

}
