...
public class QueryOperator_Menor extends QueryOperator{
	...
	protected Predicate getPredicateTrue(CriterioQuery<?> cq, String campo) {
		...
		if (value instanceof Number) {
			...
			return cb.lt(cq.getPath().get(campo).as(Number.class), number);
		}

		if (value instanceof Calendar) {
			Calendar c = (Calendar) value;
			return cb.lessThan(cq.getPath().get(campo).as(Calendar.class), c);
		}

		if (value instanceof Date) {
			Date c = (Date) value;
			return cb.lessThan(cq.getPath().get(campo).as(Date.class), c);
		}
