...
//				}
			...
			for (QueryOperator qo : ops) {
				...
				for (String campo : campos) {
					...
					if (campo.contains(".")) {
						...
						String before = StringBeforeLast.get(campo, ".");
		...
		if (!semOrderBy) {
			if (orders == null) {
				...
				for (Order order : orders) {
					String x = StringParse.get(order).replace(" first", "").trim();
					if (!StringBeforeFirst.get(x, " ").contains(".")) {
						x = "a." + x;
					}

					if (selectKeySet != null) {

						String n = StringAfterFirst.get(x, ".");
						if (n.contains(" ")) {
							n = StringBeforeFirst.get(n, " ");
						}
	...
	private String getLetra(String campo, Class<?> classe) {
		...
		String nomeCampo = StringAfterLast.get(campo, ".");
		campo = StringBeforeLast.get(campo, ".");
		...
		while (!campo.isEmpty()) {
			...
			String s = StringBeforeFirst.get(campo, ".");
			Atributo a = as.getObrig(s);
			classe = a.getType();
			as = AtributosBuild.get(classe);

			String adicionado = letras.get(s);
			if (adicionado == null) {

				letra = UConstantes.letrasMinusculas.get( letras.size() );
				letras.put(s, letra);
...
//			return StringAfterFirst.get(ts, ".");
//		} else {
//			return ts;
//		}
	}
	private MontarQueryNativa addJoin(String meio, Class<?> classe, String on) {
		String s = " join " + TableSchema.get(classe) + " on " + on;
