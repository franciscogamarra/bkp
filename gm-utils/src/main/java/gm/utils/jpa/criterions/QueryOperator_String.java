...
public abstract class QueryOperator_String extends QueryOperator {
	...
	protected Predicate getPredicateTrue(CriterioQuery<?> cq, String campo) {
		...
		if (CASE_SENSITIVE) {
			return cb.like(cb.lower(cq.getPath().get(campo).as(String.class)), getValue().toLowerCase());
		} else {
			return cb.like(cb.lower(cq.getPath().get(campo).as(String.class)), getValue().toLowerCase());
		}
	...
	protected Predicate getPredicateFalse(CriterioQuery<?> cq, String campo) {
		CriteriaBuilder cb = cq.getCb();
		return cb.notLike(cb.lower(cq.getPath().get(campo).as(String.class)), getValue().toLowerCase());
	}
	...
	protected Criterion criterion() {
		...
		//TODO Francisco
		//TODO não remover, pois estah em fase de aperfeicoamento
	...
	protected String nativo() {
		String s = getReplaced();
		s = s.replace("[ESPACO]", " ");
		return "lower( " + getCampo() + " ) SIMILAR TO '" + s + "'";
	}
