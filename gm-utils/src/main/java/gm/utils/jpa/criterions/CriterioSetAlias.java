...
public class CriterioSetAlias {
	...
	private static Atributo notExpressionAtributoCriterion = AtributosBuild.get(NotExpression.class).get("criterion");
	...
	public static void set(Criteria c, ListString aliases, List<Order> orderBy) {
		...
		for (Object expression : expressions) {
			...
			for (String s : campos) {
				...
				String alias = StringPrimeiraMaiuscula.exec(s);
			...
			campo = StringAfterLast.get(campo, ".");
	...
	public static String addAlias(ListString superAliases, String campo, Criteria c) {
		...
		campo = campo.substring(0, campo.length() - 1);
		campo = StringAfterLast.get(campo, ".");
		return hierarquia + "." + campo;
	}

	protected static Lst<Criterion> getExpressions(Criteria c) {
		@SuppressWarnings("unchecked")
		List<CriterionEntry> list = (List<CriterionEntry>) UExpression.getValue("criterionEntries", c);
		Lst<Criterion> expressions = new Lst<>();
	...
	public static String putAliasInEL(String campo) {
		...
		for (String string : campos) {
			campo += StringPrimeiraMaiuscula.exec(string) + ".";
		}
		campo += ultimo;
		return campo;
	}
