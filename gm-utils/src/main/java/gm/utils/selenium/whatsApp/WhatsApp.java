...
//		browser.setClosed(false);
	...
	private Map<String, String> getNovasMensagens() {
		...
		try {
			...
			for (WebElement div : divs) {
				...
				String key = StringTrim.plus(div.getAttribute("data-pre-plain-text"));
				if (StringEmpty.is(key) || key.toLowerCase().contains("gamarra")) {
					continue;
				}
	...
	private void digitarNomeDoContato() {
		...
		do {
			try {
				o = browser.byTitle("input", "Procurar ou começar uma nova conversa");
			} catch (Exception e) {}
		} while (o == null);
		o.sendKeys(contato);
	}
