...
//		}
	...
	public static boolean isData(Class<?> c) {
		if (c.equals(java.util.Calendar.class) || c.equals(java.util.GregorianCalendar.class) || c.equals(MyCalendar.class) || c.equals(java.util.Date.class)) {
			return true;
		}
		if (c.equals(java.sql.Date.class) || c.equals(java.sql.Timestamp.class) || c.equals(Data.class)) {
			return true;
		}
		return false;
	}

	private static final SimpleDateFormat fYear = new SimpleDateFormat("yyyy");
	private static final SimpleDateFormat fMonth = new SimpleDateFormat("MM");
	...
	public static String javaToJsDate(Date data){
		...
		String y = fYear.format(data.getTime());
		String m = fMonth.format(data.getTime());
		String restante = f.format(data.getTime());
		String dataRet = y + ","+ (Integer.parseInt(m)-1) +", "+ restante;
	...
	public static Date minusDays(Date date, int count) {
		return toDate(toLocalDate(date).minusDays(count));
	}
	...
	public static Date buldDate(int year, int month, int day) {
		return buildCalendar(year, month, day).getTime();
	}

}

//http://apgit.cooperforte.coop/cooperforte/cooperforte-backend-extrato/merge_requests/21
//http://apgit.cooperforte.coop/cooperforte/cooperforte-facade-extrato/merge_requests/22
