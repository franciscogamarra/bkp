...
public class UList {
	...
	private static Atributos getAtributos(List<?> list) {
		...
		return AtributosBuild.get( UClass.getClass( list.get(0) ) );
	}
	...
	public static void sortByMethod(List<?> list, String nomeMetodo) {
		...
		try {
			...
			Metodo metodo = getMetodo(list, nomeMetodo);
	...
	public static void orderByField(List<?> list, List<String> fields) {

		if ( isEmpty(list) || list.size() == 1 || isEmpty(fields) ) {
			return;
		}
	...
	public static <T> Lst<T> copy(List<T> list) {
		Lst<T> copy = new Lst<>();
		copy.addAll(list);
		return copy;
	}
	...
	public static void toNomeProprio(List<?> list, Atributo a) {
		for (Object o : list) {
			String s = a.getString(o);
			s = ValidadorNomePessoaFisica.instance.formatParcial(s);
			a.set(o, s);
		}
	...
	public static void remove(List<?> list, String field, Object value) {
	...
	public static <T> Lst<T> asList(T[] os) {
		if (os == null) {
			return new Lst<>();
		}
		//obs: não usar Arrays.asList
		Lst<T> list = new Lst<>();
		Collections.addAll(list, os);
		return list;
	}
	...
	public static void sort(List<?> list, Atributo atributo) {
		Comparator<Object> c = (a, b) -> UCompare.compare(a, b, atributo);
		list.sort(c);
	}

	public static <T> Lst<T> removeEmptys(Collection<T> list) {
		if (list == null) {
			return null;
		}
		Lst<T> l = new Lst<>();
	...
	public static Atributo getId(Collection<?> list) {
		...
		return AtributosBuild.get(classe).getId();
	}
	/* retorna os itens que estão respectivamente em ambas as listas */
	...
	public static <T> Lst<T> filter(List<T> list, Predicate<T> predicate) {
	...
	public static <T> T filterUnique(List<T> list, Predicate<T> predicate) {
		return filterUnique(list, predicate, null);
	}

	public static <T> T filterUnique(List<T> list, Predicate<T> predicate, String message) {
		list = filter(list, predicate);
		if (list.isEmpty()) {
			return null;
		}
		...
		throw new RuntimeException("A lista retornou + de 1 resultado (2): " + message);
	}
	...
	public static <T,TT> Lst<T> map(TT[] list, F1<TT,T> func) {
		return map(asList(list), func);
	}

	public static <T,TT> Lst<T> map(List<TT> list, F1<TT,T> func) {
		Lst<T> result = new Lst<>();
		list.forEach(o -> result.add(func.call(o)));
		return result;
	}

	public static <T,TT> Lst<T> map(List<TT> list, F2<TT,Integer,T> func) {
		Lst<T> result = new Lst<>();
		int index = 0;
		for (TT o : list) {
	...
	public static <T> Lst<T> distinct(Collection<T> list) {
		Lst<T> lst = new Lst<>();
		for (T t : list) {
			if (!lst.contains(t)) {
				lst.add(t);
	...
	public static <TARRAY, TRESULT> Lst<TRESULT> distinct(List<TARRAY> array, F1<TARRAY,TRESULT> func) {
		Lst<TRESULT> list = new Lst<>();
		array.forEach(o -> {
	...
	public static <RESULT, T> RESULT reduce(List<T> list, F2<RESULT,T,RESULT> func, RESULT startValue) {
		for (T t : list) {
