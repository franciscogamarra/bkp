...
public class UMap {
	...
	public static List<Map<String, Object>> toMap(List<?> list) {
		...
		for (Object o : list) {
			if ( UType.isList(o) ) {
				throw UException.runtime("Objeto é uma Lista");
			}
			maps.add(toMap(o));
		}
		return maps;
	...
	public static Properties asProperties(String s, String delimiter) {
		...
		for (String string : list) {
			String key = StringBeforeFirst.get(string, "=");
			String value = StringAfterFirst.get(string, "=");
			p.put(key, value);
		}
		return p;
	}
