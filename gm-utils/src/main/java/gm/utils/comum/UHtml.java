...
//	http://tunes.org/wiki/html_20special_20characters_20and_20symbols.html
	...
	public static void main(String[] args) {
		System.out.println( replaceSpecialChars("çã") );
	}
	...
	public static long romanoToDecimal(String romano) {
		String[] r = { "CM", "CD", "XC", "XL", "IX", "IV", "M", "D", "C", "L", "X", "V", "I" };
		int[] d = { 900, 400, 90, 40, 9, 4, 1000, 500, 100, 50, 10, 5, 1 };
		String valor = romano;
		...
		long total = 0;
		for (Object element : decimais) {
			if (element.toString() != null) {
				total += IntegerParse.toIntDef(element.toString(), 0);
			}
		}
		return total;
	}
	...
	public static String removeAtributos(String s) {
		...
		return removeAtributosTag("span", s);
	}

	public static String removeAtributosTag(String tag, String s){
		while (StringContains.is(s, "<"+tag+" ")) {
			String before = StringBeforeFirst.get(s, "<"+tag+" ");
			String x = StringAfterFirst.get(s, "<"+tag+" ");
			x = StringAfterFirst.get(x, ">");
			s = before + "<"+tag+">" + x;
		}
		return s;
	}
