...
public class EmailSender {
	...
	public final Multipart multipart = new MimeMultipart();
	...
	public void setAssunto(String s) {
		if (StringEmpty.is(s)) {
			throw UException.runtime("A assunto não pode ser branco");
		}
		try {
	...
	public void addDestinatario(String s) {
		if (!UEmail.isValid(s)) {
			throw UException.runtime("O email '" + s + "' parece não ser válido!" );
		}
		...
		try {
			...
			throw UException.runtime("O email '" + s + "' não passou na validação!");
		}
	}
	...
	public void addAnexo(File file, String nomeParaEnvio) {
		...
		try {
			addAnexo(new FileDataSource(file), nomeParaEnvio);
		} catch (Exception e) {
			throw UException.runtime(e);
		}
	}

	public void addAnexo(byte[] bytes, String mimeType, String nomeParaEnvio) {
		try {
			addAnexo(new ByteArrayDataSource(bytes, mimeType), nomeParaEnvio);
		} catch (Exception e) {
			throw UException.runtime(e);
		}
	}

	public void addAnexo(DataSource source, String nomeParaEnvio) {
	...
	public void send() {
		...
		if (destinatarios == null) {
			throw UException.runtime("A lista de destinatários está vazia");
		}
		if (!assunto) {
	...
	public void sendAssincrono(P0 onSucess, P1<Exception> onFail) {
	...
	public static void main(String[] args) throws MessagingException {
		...
		EmailSender mail = new EmailSender("noreply.cooperforte@cf.coop.br", null);

		mail.setAssunto("assunto teste");
		mail.setNomeRemetente("Cooperforte");
...
//		mail.addAnexo(new File("/java/m21/settings.xml"), "Outro arquivo");
		mail.addDestinatario("francisco.gamarra@gmail.com");
		mail.send();

	}

//	exemplo
	public static void main0(String[] args) throws MessagingException {
