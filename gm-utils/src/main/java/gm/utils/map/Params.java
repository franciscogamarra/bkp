...
public class Params implements Iterable<String> {
	...
	public String getString(String key){
		return StringParse.get( map.get(key) );
	}
	public ListInteger getInts(String key){
		return map.getInts(key);
	}
	public int getInt(String key, int def){
		return IntegerParse.toIntDef(map.get(key), def);
	}
	...
	public void add(String key, Object value) {
		...
		if (key.contains(".")) {
			String s = StringBeforeFirst.get(key, ".");
			Object o = get(s);
			if (o != null && (o instanceof Params || o instanceof Map) ) {
				key = StringAfterFirst.get(key, ".");
				Params sub = getSub(s);
				sub.add(key, value);
				set(s, sub.getMap());
				return;
	...
	public Params read(String s) {
		...
		for (String x : list) {
			String key = StringBeforeFirst.get(x, "=");
			String value = StringAfterFirst.get(x, "=");
			set(key, value);
		}
	...
	public Params read(Object o) {
		...
		Atributos as = AtributosBuild.get(o.getClass());
		for (Atributo a : as) {
			...
			if (a.is(BigDecimal.class) && (value instanceof String)) {
	...
	public void readPrimitivo(Object o) {
		Atributos as = AtributosBuild.get( UClass.getClass(o) );
	...
	public Params getSub(String key) {
		...
		throw UException.runtime("Não é um map!" + key + " - " + o + " - " + o.getClass());
	}
	...
	public Data getDataObrig(String key) {
		...
		try {
			...
			throw new MessageException ("Data inválida: " + getString(key));
		}
		if (data == null) {
	...
	public Numeric2 getNumeric2Obrig(String key) {
		...
		if (o == null || o.isZero()) {
			throw UException.runtime("Obrigatório: " + key);
		}
		return o;
	}
