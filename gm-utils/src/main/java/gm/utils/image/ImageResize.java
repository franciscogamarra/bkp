...
public abstract class ImageResize {
	...
	public boolean exec(){
		UAssert.notEmpty(getOrigem(), "Origem não informada");
		UAssert.notEmpty(getDestino(), "Destino não informado");
		...
		UAssert.isTrue(file.exists(), "O arquivo de origem não existe");
		try {
			execute();
			return true;
		} catch (Exception e) {
			throw UException.runtime("Não foi possível converter o arquivo", e);
		}
	}
